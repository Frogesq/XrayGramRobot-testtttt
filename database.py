...
class Database:
    ...
    def save_message(self, bc_id, msg_id, user_id, fullname, text, files_list=None, file_id=None, is_temporary=False, ttl_seconds=0, media_type=None):
        cursor = self.conn.cursor()
        files_json = json.dumps(files_list) if files_list else None
        cursor.execute("""
            INSERT OR REPLACE INTO messages (bc_id, msg_id, user_id, fullname, text, files, file_id, is_temporary, ttl_seconds, media_type)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (bc_id, msg_id, user_id, fullname, text, files_json, file_id, is_temporary, ttl_seconds, media_type))
        self.conn.commit()
